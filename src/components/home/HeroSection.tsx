...
export const HeroSection = () => {
  return (
    <section className="relative min-h-[90vh] flex items-center overflow-hidden bg-hero">
      ...
      <div className="absolute inset-0 bg-gradient-to-r from-hero via-hero/95 via-40% to-transparent" />
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            {/* Badge */}
            <motion.div 
              className="inline-flex items-center gap-2 px-4 py-2 bg-hero-foreground/10 rounded-full border border-hero-foreground/20"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.2, duration: 0.6 }}
            >
              <Sparkles className="w-4 h-4 text-accent" />
              <span className="text-sm font-medium text-hero-foreground">100% Natural & Ayurvedic</span>
            </motion.div>

            {/* Heading */}
            <motion.h1 
              className="font-serif text-4xl md:text-5xl lg:text-6xl font-bold leading-tight text-hero-foreground"
            ...
            {/* Paragraph */}
            <motion.p 
              className="text-lg text-hero-foreground/80 max-w-xl leading-relaxed"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              {[
                ...
                <motion.div 
                  key={badge.label} 
                  className="flex items-center gap-2 text-sm text-hero-foreground/80"
                  initial={{ opacity: 0, x: -20 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: 0.9 + index * 0.1, duration: 0.4 }}
                >
                  <span className="w-7 h-7 rounded-full bg-hero-foreground/10 flex items-center justify-center text-xs border border-hero-foreground/20">
                    {badge.icon}
                  </span>
                  {badge.label}
                </motion.div>
